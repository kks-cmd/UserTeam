...
import com.css.app.base.sdict.service.DictMan;
...
public class AttachService {
	...
	public static void delFiles(Attachment item) {
		try {
			...
			String category = ExtService.getCategory(item.getFileExt());
			if ("pic".equals(category)) {
				f = new File(AttachService.getFilePath(item) + "." + item.getFileExt());
				if (f.exists())
					f.delete();
				f = new File(AttachService.getFilePath(item) + ".0." + item.getFileExt());
				if (f.exists())
					f.delete();
				if (StringHelper.isNotEmpty(item.getExtraData())) {
					SlwJson jsonObj = SlwJson.parseObject(item.getExtraData());
					int number = jsonObj.getInt("number");
					for (int i = 1; i <= number; i++) {
						f = new File(fileName + "." + i + "." + item.getFileExt());
						if (f.exists())
							f.delete();
					}
