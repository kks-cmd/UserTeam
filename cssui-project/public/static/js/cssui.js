...
(function($) {
	...
	$.fn.contextMenu = function(id, options) {
		if (!(id in menuMap))
			menuMap[id] = $('#' + id).clone(true).appendTo('body');
		var menu = menuMap[id];
		var a = $(this);
...
;(function($){
 	$newTable = {
	    ...
	    $('body').on('click','#ids',function(){
	        var $this=$(this),
	            $form=$this.parents('.table-form'),
	            $idsLen=$form.find('input[id="ids"]').length,
	            $idsChkLen=$('input[id="ids"]:checked').length;
	        if($idsLen == $idsChkLen){
	        	$form.find('.cleck-all').prop("checked",$this.prop("checked"));
	        }else{
	        	$form.find('.cleck-all').prop("checked",false);
	        }
	    })
...
(function($) {
	var actMessage = {
		...
		template : function(data, params, form) {
			if (params.callback) {
				params.callback(data, form);
				return;
			}
			if (data.result == 0) {
				if (isnull(params.afterExec)) // refreshCurrentTabForm,
					// refreshCurrentTab
					params.afterExec = refreshCurrentTabForm;
	...
	$action = {
		...
		submit : function(form, callback, tabId, afterExec) {
			...
			$css.post($form.attr("action"), $form.serialize(), function(data) {
				...
				if (isnull(params.afterExec))
					params.afterExec = closeDialogRefreshTabForm;
				methods.template(data, params, form);
			}, 'json');
			return false;
		},
...
$sidebar = {
	...
	collapse : function() {
		$sidebar.obj.find('span').css("display", "none");
		$('.wrapper-left').css('width', '48px');
		$('.wrapper-body-2').css('margin', '0 0 0 48px');
		$sidebar.startEvent();
		$sidebar.activeTopLi();
	},
	startEvent : function() {
	...
	init : function(menu) {
		...
		$(menu + ':not(.sidebar-menu)').bind('dblclick', function(e) {
			var $target = $(e.target);
			if ($target.attr('class') == 'sidebar l-autoscroll') $sidebar.toggle();
		});
...
function blockUI(el, centerY) {
	var el = jQuery(el);
	el.block({
		message : '<img src="' + _webroot + 'static/cssui/images/ajax-loading.gif" align="">正在努力的加载,请稍等......',
...
///<jscompress sourcefile="jqdict.js" />
/**
